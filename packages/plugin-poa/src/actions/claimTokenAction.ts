import { type IAgentRuntime, type Memory, type State, type HandlerCallback, elizaLogger, composeContext, generateObjectDeprecated, ModelClass } from "@elizaos/core";
...
1. Identify the relevant information from the user's message, not from the agent, only use the message within 1 min:
   - Quote the message mentioning the assets.
     - only use the message within 1 min
...
6. If there some reason that you can't extract the data, please guide the user how to provide it, and put it to the <hint> tags.
...
export const claimTokenAction = {
    ...
    handler: async (
        ...
        try {
            ...
            const poaRelay = runtime.getSetting("POA_RELAY") ? runtime.getSetting("POA_RELAY") : "https://proof-of-agent-relay.ata.network";
            ...
            elizaLogger.log(`content: ${output}`);
